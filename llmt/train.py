...
from .config import RUNS_DIR, DTYPE, get_device, get_mixed_precission
...
def train(batch_size: int = 12,
    ...
    accelerator = Accelerator(gradient_accumulation_steps=gradient_accumulation_steps,
                              mixed_precision=get_mixed_precission(dtype))
    ...
    if accelerator.is_main_process:
        writer = configure_logger(config, experiment_dir)
    ...
    # initial metric
    best_score = test_epoch(accelerator, model, tokenizer, val_dataloader, context_size, finetune,
                            writer, device=device, epoch=0, global_step=0)
    ...
    for e in range(epochs):
        ...
        epoch(accelerator, train_dataloader, tokenizer, e, model, optimizer, lr_scheduler,
              finetune, gradient_clip, writer, batch_size=accelerator.num_processes * batch_size,
              dtype=dtype)

        progress_bar.update(1)
        if accelerator.is_main_process:
            lr_scheduler.step()
        score = test_epoch(accelerator, model, tokenizer, val_dataloader, context_size, finetune,
                           writer, device=device, epoch=e, global_step=(e + 1) * size_dataset)
...
def configure_logger(config: Dict, experiment_dir: Path) -> SummaryWriter:
    """
